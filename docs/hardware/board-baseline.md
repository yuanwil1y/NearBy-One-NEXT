...
5. The same repository's current ESP-IDF LVGL example (`02_Example/ESP-IDF/07_LVGL_Test/main/main.c`) calls a copied `esp_lcd_new_panel_sh8601()` wrapper, but its command table programs ST7789-style porch/VCOM/power/gamma registers and conflicts with the official product documentation and working Arduino example only in the wrapper name. Treat the SH8601 symbol as an example bug / misnamed vendor wrapper, not board identity.
6. Espressif upstream provides the public `esp_lcd_new_panel_st7789()` driver. In ESP-IDF v6.1 that generic driver's init covers reset/sleep-out, MADCTL, COLMOD and RAMCTRL, but does not program the Waveshare board's porch/power/gamma values.

## Settled LCD path

For this board, use the ESP-IDF public ST7789 driver for panel ownership/drawing and keep the Waveshare board-specific tuning commands as a small explicit board init step:
...
SPI2_HOST
  ...
  -> esp_lcd_panel_reset()
  -> esp_lcd_panel_init()
  -> Waveshare board tuning via esp_lcd_panel_io_tx_param()
...
Panel geometry is 170x320 RGB565. The visible window starts at controller column offset 35 and row offset 0 in portrait orientation. Do not copy the `esp_lcd_sh8601.*` files into NearBy One; only retain the board register values that the Waveshare ESP-IDF example applies to the physical panel.
...
The Waveshare Arduino ST7789 library and the Waveshare ESP-IDF example use slightly different analog/gamma values and inversion defaults. The current NearBy One baseline intentionally follows the Waveshare **ESP-IDF board sequence** for those panel-specific values because it matches the same SDK/IO path. The physical four-corner/color test remains the final authority before marking the LCD path bench-verified.
...
1. initializes ST7789 via the public ESP-IDF driver plus the Waveshare board tuning step;
2. applies `(35, 0)` gap;
3. draws distinct colors/pixels/rectangles at all four logical corners and a grayscale/color ramp to detect wrong inversion/gamma;
...
If the four-corner pattern is aligned without LVGL-side coordinate hacks and the color/inversion test is correct, the ST7789V2 + board tuning + 35-column offset path is the canonical NearBy One BSP path.
