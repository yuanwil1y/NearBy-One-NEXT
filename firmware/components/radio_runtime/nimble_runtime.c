...
{
    if (event != NULL && event->type == BLE_GAP_EVENT_DISC_COMPLETE) {
        s_scanning = false;
    }
...
esp_err_t nearby_nimble_driver_init(TickType_t sync_timeout_ticks)
{
    ...
    esp_err_t err = nearby_ble_handoff_init();
    if (err != ESP_OK) {
        return err;
    }
    ...
    if (err != ESP_OK) {
        /* Do not erase NVS automatically from this hardware layer. */
        return err;
    }
    ...
    if (xSemaphoreTake(s_sync_sem, sync_timeout_ticks) != pdTRUE || !s_ready) {
        (void)nearby_nimble_driver_deinit();
...
esp_err_t nearby_nimble_driver_deinit(void)
{
    ...
    if (s_scanning) {
        first_err = nearby_nimble_scan_stop();
    }

    int rc = nimble_port_stop();
    if (rc != 0 && first_err == ESP_OK) {
        first_err = ESP_FAIL;
    }
    if (rc == 0) {
