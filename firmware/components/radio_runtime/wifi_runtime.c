...
static esp_err_t require_radio_owner(void)
{
    if (scan_session_require_scan_owner() == ESP_OK ||
        scan_session_require_competing_owner() == ESP_OK) {
        return ESP_OK;
    }
...
esp_err_t nearby_wifi_driver_init(void)
{
    ...
    esp_err_t err = nearby_wifi_handoff_init();
    if (err != ESP_OK) {
        return err;
    }
    ...
    init_cfg.nvs_enable = false; /* v0.1 credentials are session-only. */
...
esp_err_t nearby_wifi_driver_deinit(void)
{
    ...
    if (first_err == ESP_OK && err != ESP_OK && err != ESP_ERR_WIFI_NOT_STARTED) {
        first_err = err;
    }
