...
static uint8_t s_lan_rx[KISMET_LAN_RX_MAX];
static char s_mdns_types[KISMET_MDNS_DISCOVERED_TYPE_MAX]
                        [WIRESHARK_MDNS_NAME_MAX + 1];
...
static bool mdns_add_type(const char *type, uint8_t *count)
{
    ...
    if (*count >= KISMET_MDNS_DISCOVERED_TYPE_MAX) {
        return false;
    }
    const size_t n = bounded_strlen(type, WIRESHARK_MDNS_NAME_MAX + 1u);
    if (n == 0 || n > WIRESHARK_MDNS_NAME_MAX) {
        return false;
    }
...
static bool dns_encode_name(uint8_t *buf, size_t cap, size_t *off,
                            const char *name)
{
    ...
    while (*p != '\0') {
        ...
        const size_t label_len =
            dot != NULL ? (size_t)(dot - p) : strlen(p);
        if (label_len == 0) {
            if (dot != NULL && dot[1] == '\0') {
                p = dot + 1;
                break;
...
static esp_err_t mdns_send_ptr_query(int fd, const char *name)
{
    ...
    query[5] = 1;
    size_t off = 12;
    if (!dns_encode_name(query, sizeof(query), &off, name) ||
        off + 4u > sizeof(query)) {
...
static esp_err_t mdns_handle_bytes(
    const kismet_mdns_scan_config_t *config,
    kismet_mdns_scan_stats_t *stats,
    const uint8_t *data,
    size_t len,
    uint8_t *type_count)
{
    wireshark_mdns_facts_t facts;
    const wireshark_lan_parse_result_t parsed =
        wireshark_mdns_parse(data, len, &facts);
    ++stats->datagrams;
    if (parsed == WIRESHARK_LAN_PARSE_ERR_MALFORMED ||
        parsed == WIRESHARK_LAN_PARSE_ERR_ARG) {
        ++stats->malformed_datagrams;
        return ESP_OK;
    }
    if (parsed == WIRESHARK_LAN_PARSE_PARTIAL) {
        ++stats->partial_datagrams;
    }

    for (uint8_t i = 0; i < facts.service_count; ++i) {
        const wireshark_mdns_service_t *service = &facts.services[i];
        ++stats->services_seen;
        if (strcasecmp(service->service_type,
                               ...
                               kismet_mdns_scan_stats_t *stats)
{
    ...
    err = mdns_receive_until(fd, start, discovery_ticks, config, stats,
                             &type_count);
    if (err == ESP_OK) {
    ...
    if (err == ESP_OK) {
        err = mdns_receive_until(fd, start, total_ticks, config, stats,
                                 &type_count);
    }
                               ...
                               kismet_ssdp_scan_stats_t *stats)
{
    ...
    while (!elapsed(start, duration)) {
        ...
        ++stats->datagrams;
        wireshark_ssdp_facts_t facts;
        const wireshark_lan_parse_result_t parsed =
            wireshark_ssdp_parse(s_lan_rx, (size_t)rc, &facts);
        if (parsed == WIRESHARK_LAN_PARSE_ERR_MALFORMED ||
            parsed == WIRESHARK_LAN_PARSE_ERR_ARG) {
            ++stats->malformed_datagrams;
            continue;
        ...
        if (config->dedup != NULL &&
            !kismet_ssdp_dedup_should_emit(config->dedup, &facts)) {
            ++stats->duplicate_datagrams;
            continue;
        }
        if (config->observation_cb != NULL) {
            first_error = config->observation_cb(
                &facts, parsed, config->observation_ctx);
                              ...
                              kismet_dhcp_scan_stats_t *stats)
{
    ...
    while (!elapsed(start, duration)) {
        ...
        const wireshark_lan_parse_result_t parsed =
            wireshark_dhcp_parse(s_lan_rx, (size_t)rc, &facts);
        if (parsed == WIRESHARK_LAN_PARSE_ERR_MALFORMED ||
            parsed == WIRESHARK_LAN_PARSE_ERR_ARG) {
            ++stats->malformed_datagrams;
            continue;
        ...
        if (config->dedup != NULL &&
            !kismet_dhcp_dedup_should_emit(config->dedup, &facts)) {
            ++stats->duplicate_datagrams;
            continue;
        }
        if (config->observation_cb != NULL) {
