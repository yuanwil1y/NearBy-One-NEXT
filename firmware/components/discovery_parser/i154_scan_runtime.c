...
static esp_err_t consume_batch(const nearby_i154_scan_config_t *config,
    ...
    *consumed_any = false;
    for (unsigned n = 0; n < I154_BATCH_MAX; ++n) {
        ...
        *consumed_any = true;
        ...
        nearby_i154_normalized_t normalized;
        const nearby_i154_parse_result_t mac_result =
            nearby_i154_normalize_record(record, &normalized);
        nearby_zigbee_discovery_facts_t zigbee;
        memset(&zigbee, 0, sizeof(zigbee));
        nearby_zigbee_parse_result_t zigbee_result = NEARBY_ZIGBEE_PARSE_UNSUPPORTED;

        if (mac_result == NEARBY_I154_PARSE_ERR_ARG ||
            mac_result == NEARBY_I154_PARSE_ERR_MALFORMED) {
            ++stats->malformed_records;
        } else if (mac_result == NEARBY_I154_PARSE_SECURED) {
            ++stats->secured_records;
        } else if (mac_result == NEARBY_I154_PARSE_UNSUPPORTED) {
            ++stats->unsupported_records;
        } else {
            zigbee_result = nearby_zigbee_parse_from_mac(record->frame + 1u,
                                                         record->frame[0],
                                                         &normalized.mac,
                                                         &zigbee);
            if (zigbee_result == NEARBY_ZIGBEE_PARSE_SECURED) {
                ++stats->secured_records;
            } else if (zigbee_result == NEARBY_ZIGBEE_PARSE_ERR_ARG ||
                       zigbee_result == NEARBY_ZIGBEE_PARSE_ERR_MALFORMED) {
                ++stats->malformed_records;
            } else if (zigbee_result == NEARBY_ZIGBEE_PARSE_UNSUPPORTED) {
                ++stats->unsupported_records;
            } else if (zigbee.has_device_announce || zigbee.has_node_descriptor ||
                       zigbee.has_simple_descriptor || zigbee.active_endpoint_count > 0u ||
                       zigbee.has_manufacturer || zigbee.has_model) {
        ...
        if (config->observation_cb != NULL &&
            mac_result != NEARBY_I154_PARSE_ERR_ARG &&
            mac_result != NEARBY_I154_PARSE_ERR_MALFORMED) {
            callback_err = config->observation_cb(&normalized, mac_result,
                                                  &zigbee, zigbee_result,
...
static esp_err_t drain_ready(const nearby_i154_scan_config_t *config,
                             nearby_i154_scan_stats_t *stats)
{
    bool consumed = false;
